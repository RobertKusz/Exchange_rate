...
public class NbpApi {
    ...
    public String getDataFromTo(String code, LocalDate from, LocalDate to) {
        URL url = null;
        try {
            url = new URL("http://api.nbp.pl/api/exchangerates/rates/A/"+code+"/"+from+"/"+to+"/");
        } catch (MalformedURLException e) {
            throw new RuntimeException(e);
        }
        return getData(url);
    ...
    private static String getData(URL url) {
        ...
        int responseCode = conn.getResponseCode();
            ...
            return getDataFromAPI(conn).toString();
    ...
    private static StringBuilder getDataFromAPI(HttpURLConnection conn) throws IOException {
